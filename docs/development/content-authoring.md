...
The shared text/directive/link contract, validated local media, controlled GIF playback, and full project/article/retrospective mixed-content round trip are implemented and locally verified. Broader cross-site accessibility, SEO, and performance verification remains in the [development roadmap](../project-roadmap.md).
...
|---|---|---|
| Selected-text styling | One server-rendered detail component supports CommonMark strong/emphasis and controlled `:underline[text]`. Import rejects raw HTML and unsupported directives; rendering independently omits HTML and retains safe text. | Broader cross-site quality verification remains in DEV-10. |
| Still images and GIFs | Import and public rendering share a local-file policy for PNG, JPEG, static WebP, and GIF. Intrinsic dimensions reserve responsive space. Complete synthetic project, article, and retrospective journeys verify poster-first GIFs, keyboard Play/Stop, a new reduced-motion preference, no-JavaScript posters, and responsive asset loads. | Cross-site quality measurement remains in DEV-10; owner review remains required for release assets and alternatives. |
| GitHub and website links | Import and rendering share decoded/normalized HTTP(S), root-path, and generated-heading fragment rules. External links include a new-tab notice and `noopener noreferrer`. Body links use bold, double-underlined styling to distinguish them from underlined prose. | Owner review of external destination availability remains required. |
| Mixed content and repeated sections | The shared renderer preserves ordered CommonMark sections and maps body H1 to H2. One import-ready synthetic fixture proves ordered prose, stills, a controlled GIF, trailing prose, inline links, and closing website/source sections through PostgreSQL and all three public detail types. | Owner-reviewed release writing remains in DEV-10A. |

Evidence: [shared renderer](../../src/features/content/presentation/markdown/detail-markdown.tsx), [GIF client leaf](../../src/features/content/presentation/markdown/controlled-gif.tsx), [Markdown adapter](../../src/features/content/infrastructure/markdown/detail-markdown.ts), [media filesystem adapter](../../src/features/content/infrastructure/markdown/detail-media.ts), [import parser](../../src/features/content/infrastructure/imports/parse-content-import.ts), [local-file tests](../../tests/unit/features/content/detail-media.test.ts), [shared component tests](../../tests/component/detail-markdown.test.tsx), [PostgreSQL import tests](../../tests/integration/imports/postgres-content-import.test.ts), and [production-server browser checks](../../tests/e2e/detail-markdown.spec.ts).

The importer checks body type, length, NUL characters, frontmatter, reserved fixture markers, parsed directives/HTML, decoded link destinations, and every referenced media asset before opening the import transaction in either mode. Media inspection validates the normalized content-owned path, real file location, actual encoded format, nonzero dimensions, byte budgets, nonempty alternative text, and GIF poster pairing without fetching a URL. Automated checks establish that alternative text is present; an owner must still review whether it is useful in context.
...
### Asset preparation and read-only verification

1. Export the reviewed source at its intrinsic display dimensions as PNG, JPEG, static WebP, or GIF. Use lowercase extensions and a stable filename under `public/media/<content-slug>/`; do not use symlinks to files outside `public/media/`.
2. For `name.gif`, export a representative static frame with exactly the same pixel dimensions as `name.poster.webp`. Confirm that the poster alone communicates enough context for a visitor who does not play animation.
3. Write concise alternative text that communicates the relevant evidence rather than the filename or decorative appearance. Review it manually in the surrounding narrative.
4. Run the read-only verifier before an import dry-run. It parses the same version-1 document and reports per-body unique asset counts and bytes without connecting to PostgreSQL or fetching remote content.

```bash
pnpm content:media:verify --file path/to/reviewed-content.json
pnpm content:import --target development --file path/to/reviewed-content.json --dry-run
```

The explicitly synthetic [media policy input](../../content/examples/media-policy-demo.json) and its [generated project asset notice](../../public/media/media-policy-demo/README.md) provide a safe local probe, not professional evidence. The project, article, and retrospective each contain three references resolving to four content-owned files because the GIF includes its poster. Each body uses 97,258 bytes; the fixture uses 291,774 bytes across 12 unique assets, and every file remains below its individual budget. The example assets can be reproduced after installing the pinned Playwright Chromium build:

```bash
node --import tsx scripts/generate-synthetic-media.mts --allow-write
pnpm content:media:verify --file content/examples/media-policy-demo.json
```

Regeneration deliberately requires `--allow-write`; the verifier is the default read-only release check. Version the JSON and referenced assets together, retain files still referenced by published bodies, and package `public/media/` with the application before publishing the matching content.

## Mixed-content example

The JSON `title` supplies the page title, for example `Offline Synchronization Case Study`. Its `body` can contain the following ordered Markdown. These particular offline-sync media references remain illustrative placeholders rather than existing files or an import-ready bundle; the separate synthetic media-policy input is runnable for a project, article, and retrospective. Replace the placeholders with reviewed assets and a matching GIF poster before importing this body.
...
Project/post presentation uses one controlled detail renderer in the `content` feature. Its pure URL policy lives in domain code; shared Markdown and filesystem adapters live in infrastructure and are used directly by the outer presentation and import adapters. Neither parser, filesystem, nor React types enter application/domain code. `remark-directive` 4.0.0 supplies syntax-tree parsing with correct escapes and code boundaries, not author-selected HTML; the project handles only underline. `remark-parse` 11.0.0 and `unified` 11.0.5 are explicit direct dependencies matching the existing renderer's parser stack, and `@types/mdast` 4.0.4 supplies development-only tree types. This avoids a second regex-based Markdown grammar. Detail text and initial posters remain server-rendered; only the Play/Stop state is a client leaf. No image-processing runtime dependency was added: the adapter inspects bounded local file headers and the browser serves versioned assets directly.

The PostgreSQL `body` text column and version-1 import shape are unchanged. Before activating text validation, a read-only audit found no text/link incompatibilities in 7 example JSON bodies, 9 development database bodies, and 8 isolated test database bodies. The mixed-content fixture now adds three explicitly synthetic import-ready bodies and does not rewrite owner-authored data. Incoming bodies are checked separately from stored snapshot schemas, so a reviewed valid import can repair a legacy body; dry-run never rewrites it. Public rendering repeats filesystem validation so a legacy/modified row or removed file produces readable alternative fallback text instead of an unsafe or broken active image.

Unit/component coverage includes supported image formats, dimensions and budgets, empty alternatives, content-type mismatch, missing and oversized files, encoded traversal/separators, symlink escape, GIF poster pairing, safe render fallbacks, poster-first output, Play/Stop, and a runtime reduced-motion change. PostgreSQL tests cover valid dry-run/apply/repeat, stable identities, normalized readback through every public repository, unrelated-row/relation preservation, and invalid text/link/media zero-write behavior through the application role and real CLI. Production-server Chromium checks load each detail's actual assets at 375, 768, and 1440 pixel widths, assert one H1, authored section/media/prose order, intrinsic dimensions and no overflow, prove that each GIF request begins only after keyboard activation, exercise Stop/replay and reduced-motion stopping, and verify poster-only no-JavaScript documents.

The focused media verification passed `./scripts/check.sh` (238 unit/component tests, read-only media verification, formatting, lint, types, migration consistency, and production build), `pnpm db:test:repositories` (35 isolated application-role integration tests), and `pnpm test:e2e` (20 production-server Chromium journeys). Manual review of the synthetic detail capture confirmed reading order, matching GIF-poster subject matter, useful alternatives for the generated shapes/movement, responsive containment, and the visible keyboard focus indicator. The generator and verifier also reject missing explicit write authorization and a missing input with exit code 2. These checks use only synthetic test data and make no production-readiness claim.

The complete mixed-content workflow passed the same 238 unit/component checks, 35 isolated application-role integration tests, and 20 production-server Chromium journeys. The runnable fixture's dry-run created no rows, apply created one skill, one tag, one project, and two posts, and repeat reported every stable identity unchanged. Repository readback returned the normalized body and published relationships for the project, article, and retrospective; fixture cleanup restored the prior seven-table snapshot. Manual review of desktop project, mobile article, and tablet retrospective captures confirmed section/prose/media order, visibly distinct underlined text and links, meaningful synthetic alternatives in context, poster-first GIF controls, and contained responsive layouts. External sample destinations were not fetched, and project canonical metadata remains the recorded DEV-10 gap.
...
DEV-10 performs the broader cross-site accessibility, SEO, and performance pass. DEV-10A separately requires owner-reviewed factual writing, media rights/redaction, useful alternatives, and destination availability; this synthetic workflow does not satisfy that release-content gate.
