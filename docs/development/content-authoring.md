...
The shared text/directive/link contract, validated local media, and controlled GIF playback are implemented. Full project/article/retrospective mixed-content round-trip verification remains planned in DEV-09C in the [development roadmap](../project-roadmap.md).
...
| Still images and GIFs | Import and public rendering share a local-file policy for PNG, JPEG, static WebP, and GIF. Intrinsic dimensions reserve responsive space. GIFs render a companion WebP poster until an explicit keyboard-operable Play action; Stop, a new reduced-motion preference, and no JavaScript retain or restore the poster. | DEV-09C must prove this behavior in complete project, article, and retrospective narratives. |
...
| Mixed content and repeated sections | The shared renderer preserves ordered CommonMark sections and maps body H1 to H2. Text fixtures cover identical project/article/retrospective output, and the synthetic media-policy input interleaves prose, stills, a controlled GIF, and trailing prose. | DEV-09C must verify one reviewed import and public round trip across all three detail types. |
...
The explicitly synthetic [media policy input](../../content/examples/media-policy-demo.json) and its [generated asset notice](../../public/media/media-policy-demo/README.md) provide a safe local probe, not professional evidence. Its three references resolve to four unique files (the GIF includes its poster): 97,258 bytes for the body, with each file below its individual budget. The example assets can be reproduced after installing the pinned Playwright Chromium build:
...
DEV-09C still needs one complete synthetic project/article/retrospective import to prove authored mixed-content order, links, media, metadata, and repository readback together. DEV-10 then performs the broader cross-site accessibility, SEO, and performance pass. The focused DEV-09B checks do not substitute for either gate.
