...
- Home provides a native-development introduction, up to two featured projects, three skills with published project evidence, the three newest posts across both kinds, and a contact entry point. Its dynamic server reads share existing repositories, preserve available sections during partial failures, and distinguish empty content from unavailable data. About and Contact are static pages with background, working principles, current focus, and labeled sample contact links. Every route uses the same responsive public shell with semantic landmarks, a three-dimensional terminal greeting above sticky current-page navigation, visible focus, skip navigation, reduced-motion behavior, Dracula-derived presentation tokens, self-hosted D2Coding for body and code text, and self-hosted NeoDunggeunmo Code for titles.
